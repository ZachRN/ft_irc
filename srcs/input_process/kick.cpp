...
int	kick(Client* client, std::vector<std::string> parsed_input, Server *server, std::string unparsed)
{
	...
	if (kickee == nullptr)
	{
		send_msg(client->get_fd(),(":" + server->get_config().get_host() + " 401 " + client->get_nickname() + " " + parsed_input.at(2) + " :No such nick/channel\n"));
		return (FAILURE);
	}
	switch (channel->kick(kickee, client))
	{
		case CANT_KICK_OWNER:
			send_msg(client->get_fd(), ":" +server->get_config().get_serverName()+ " 400 " + client->get_nickname() + " :Unknown Can't Kick Owner.\n");
			break;
		case NOT_IN_CHANNEL:
			send_msg(client->get_fd(), ":" +server->get_config().get_serverName()+ " 400 " + client->get_nickname() + " :Unknown Can't Kick Someone Not in Channel.\n");
			break;
		default:
			break;
	}
